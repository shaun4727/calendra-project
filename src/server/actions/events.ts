...
export async function createEvent(
	...
	try {
		...
		await prisma.event.create({ data: { ...data, clerkUserId: userId } });
	} catch (error: any) {
...
export async function getEvents(clerkUserId: string): Promise<EventRow[]> {
	...
	const events = await prisma.event.findMany({
		where: {
			clerkUserId: clerkUserId, // filter events by Clerk user ID
		},
		orderBy: {
			name: 'asc', // alphabetical order (case-sensitive)
		},
	});
