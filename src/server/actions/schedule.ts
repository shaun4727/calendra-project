...
// type AvailabilityRow = Prisma.ScheduleAvailabilityGetPayload<{}>;

type AvailabilityRow = Prisma.ScheduleAvailabilityGetPayload<object>;

export type FullSchedule = Prisma.ScheduleGetPayload<{
	include: {
		availabilities: true;
	};
}>;
...
export async function saveSchedule(
	...
	try {
		...
		const upsertedSchedule = await prisma.schedule.upsert({
			where: { clerkUserId: userId },
			update: scheduleData,
			create: {
				...scheduleData,
		...
		const scheduleId = upsertedSchedule.id;

		// 2. Run deletion and optional insertion in a single transaction
		await prisma.$transaction([
			// Delete old availabilities linked to this schedule
...
export async function getValidTimesFromSchedule(
	...
	return timesInOrder.filter((intervalDate) => {
		...
		return (
			...
			eventTimes.every((eventTime: any) => {
...
function getAvailabilities(
	groupedAvailabilities: Partial<Record<(typeof DAYS_OF_WEEK_IN_ORDER)[number], AvailabilityRow[]>>,
	date: Date,
	timezone: string,
): { start: Date; end: Date }[] {
	...
	return dayAvailabilities.map(({ startTime, endTime }) => {
		...
		const start = fromZonedTime(setMinutes(setHours(date, startHour), startMinute), timezone);

		// Create an end Date object set to the correct hour and minute, then convert it to the given timezone
		const end = fromZonedTime(setMinutes(setHours(date, endHour), endMinute), timezone);
